 ...
 */
 public class EmptyTree<K extends Comparable<K>,V> implements Tree<K,V> {
	...
	public NonEmptyTree<K, V> insert(K key, V value) {
		EmptyTree<K, V> blank = EmptyTree.getInstance();
		NonEmptyTree<K, V> temp = new NonEmptyTree<K, V>(key, value, blank);
		return temp;
	}
	...
	public K max() throws TreeIsEmptyException {
		throw new TreeIsEmptyException("There is no maximum element");
	}

	public K min() throws TreeIsEmptyException {
		throw new TreeIsEmptyException("There is no minimum element");
	}
